            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('forms.fill.store', $form->id) }}" method="POST"
                        ...
                        <input type="hidden" value="{{ Request::route('formvalue') }}" name="form_value_id" />
                        @method('PUT')
                                    ...
                                    <div class="row">
                                        @foreach ($rows as $row_key => $row)
                                            ...
                                            @endphp 
                                                <div class="form-group {{ $col }}">
                                                    {{ Form::label($row->name, $row->label . ($row->required ? ' *' : ''), ['class' => 'd-block']) }}
                                                    @if($row->label=="Score by QRB" && !$qrbedit)
                                                     @php
                                                        $disb =  'disabled';
                                                    @endphp
                                                    @endif
...
<script>
 ...
 function loadDisable(elem, radVal) {
    ...
    if(elem=='q25a'){
        disbtn(radVal, 'q25a1');
        AAdisbtn(radVal, 'RefNumber-q25a1');
