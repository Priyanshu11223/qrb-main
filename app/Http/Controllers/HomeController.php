    ...
    public function index()
    {
        ...
        if (!file_exists(storage_path() . "/installed")) {
            ...
            if ($usr->type == 'Admin') {
               ...
               $userForm = FormValue::select(['form_values.*', 'forms.title'])
                    ->join('forms', 'forms.id', '=', 'form_values.form_id')
                    ->leftJoin('users', 'users.id', 'form_values.user_id')
                    ->where('form_values.user_id',$usr->id)->first();    
            ...
            if($userForm){
            $editFormURL = route('uedit.form.values', $userForm->id);
            }
            return view('dashboard/home', compact('user', 'form', 'submitted_form', 'userForm', 'editFormURL'));
        }
    }
